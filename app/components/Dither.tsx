...
vec3 dither(vec2 uv, vec3 color, vec2 res) {
...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
  ...
  color.rgb = dither(uv, color.rgb, resolution);
  outputColor = color;
}
`;
...
const RetroEffect = forwardRef<RetroEffectImpl, { colorNum: number; pixelSize: number }>(
  ({ colorNum, pixelSize }, ref) => <WrappedRetroEffect ref={ref} colorNum={colorNum} pixelSize={pixelSize} />,
);
...
function DitheredWaves({
  ...
  useEffect(() => {
    ...
    waveUniformsRef.current.resolution.value.set(Math.floor(size.width * dpr), Math.floor(size.height * dpr));
  }, [size, gl]);
